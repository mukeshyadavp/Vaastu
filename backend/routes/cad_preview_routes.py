...
from ai.cad_preview_service import generate_dxf_preview
...
def create_cad_preview():
    ...
    if file_ext != ".dxf":
        return jsonify({
            "success": False,
            "message": "Preview is currently supported only for DXF files",
        }), 400

    upload_folder = current_app.config["UPLOAD_FOLDER"]
    preview_folder = os.path.join(current_app.config["BASE_DIR"], "generated_previews")
    ...
    try:
        preview_filename = generate_dxf_preview(file_path, preview_folder)
